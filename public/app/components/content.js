...
class Content extends Component {
    render() {
        ...
        return (
            <ul>
                {items.map( (v,key )=> <LiItem filter={this.props.filter} item={v} key={key} deleteItem={this.props.deleteItem} />)}
            </ul>
        )
    }
}
    ...
    render() {
        ...
        return (
            <li className={liClass}>
                <span style={{marginRight: '4px'}}>{this.props.item.num}</span>
                  <span style={{marginRight: '4px'}}>{this.props.item.name}</span>
